...
let container, player, camera, cameras, scene, renderer;
let animation = {};
...
function Game(){	
	...
	options.assets.push(`${game.assetsPath}ifc/05.ifc`);
	...
	{set activeCamera(object){
		cameras.active = object;
	}}
	
	function init() {
				
		mode = modes.INITIALISING;
		...
		ifcloader.ifcManager.setWasmPath( '/assets/ifc/web-ifc.wasm' );

		let mixer = null
		
		const player = new PlayerLocal(this);
		
		loadEnvironment(loader, ifcloader);
		
		const speechBubble = new SpeechBubble(this, "", 150);
		speechBubble.mesh.position.set(0, 350, 0);
		...
		if ('ontouchstart' in window){
			window.addEventListener( 'touchdown', (event) => onMouseDown(event), false );
		}else{
	...
	function loadEnvironment(loader, ifcloader){
		ifcloader.load(`${this.assetsPath}ifc/05.ifc`, 
		(model) =>
		{
			model.colliders = [];
			scene.add(model);
	...
	function loadNextAnim(loader){
		...
		loader.load( `${this.assetsPath}fbx/anims/${anim}.fbx`, function( object ){
			player.animations[anim] = object.animations[0];
			if (anims.length>0){
				loadNextAnim(loader);
			}else{
				delete anim.anims;
	...
	function playerControl(forward, turn){
		...
		if (forward>0.3){
			if (player.action!='Walking' && player.action!='Running') player.action = 'Walking';
		}else if (forward<-0.3){
			if (player.action!='Walking Backwards') player.action = 'Walking Backwards';
		}else{
			forward = 0;
	...
	function createCameras(){
		...
		cameras = { front, back, wide, overhead, collect, chat };
		activeCamera = cameras.back;	
	}
	
	function showMessage(msg, fontSize=20, onOK=null){
	...
	function updateRemotePlayers(dt){
		if (remoteData===undefined || remoteData.length == 0 || player===undefined || player.id===undefined) return;
		...
		this.remoteData.forEach( function(data){
			if (player.id != data.id){
				...
				let iplayer;
				initialisingPlayers.forEach( function(player){
				...
				if (iplayer===undefined){
					...
					if (rplayer===undefined){
						//Initialise player
						initialisingPlayers.push( new Player( this, data ));
					}else{
						//Player exists
	...
	function getRemotePlayerById(id){
